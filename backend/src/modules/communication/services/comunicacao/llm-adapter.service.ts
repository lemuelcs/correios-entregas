...
export const wppPilotLlmAdapter = {
  async processUnmatchedMessage(
    ...
    const config = await db.wppPilotConfig
      .findFirst({ where: { unidadeId: dspId }, select: { id: true, llmEnabled: true, llmConfig: true } })
      .catch(() => null);
    ...
    const history = await _getRecentHistory(phone, 10);
...
async function _logUsage(
  ...
  const wppConfig = await db.wppPilotConfig
    .findFirst({ where: { unidadeId: dspId }, select: { id: true } })
    .catch(() => null);
