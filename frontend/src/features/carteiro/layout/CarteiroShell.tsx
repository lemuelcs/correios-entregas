...
import { getCarteiroPageMeta } from '../carteiro.config';
import { CarteiroBottomNav } from './CarteiroBottomNav';
import { CarteiroTopBar } from './CarteiroTopBar';

export function CarteiroShell() {
  useAuthGuard('CARTEIRO');

  const location = useLocation();
  const navigate = useNavigate();
  const logout = useAuthStore((state) => state.logout);
  const page = getCarteiroPageMeta(location.pathname);
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(214,228,255,0.92),_transparent_32%),linear-gradient(180deg,_#f3f5fa_0%,_#e2e8f0_100%)] px-0 py-0 md:px-6 md:py-6">
      <div className="mx-auto flex min-h-screen max-w-[390px] flex-col overflow-hidden bg-white shadow-[0_30px_80px_rgba(15,23,42,0.16)] md:min-h-[844px] md:rounded-[34px] md:border md:border-slate-300/80">
        ...
        {page.showBottomNav ? <CarteiroBottomNav activeTab={page.bottomTab} /> : null}

        <div className="flex h-6 items-center justify-center bg-white md:bg-white">
