...
/**
 * Most useful suffixes for database matching across varied phone formats.
 */
export function phoneMatchSuffixes(phone: string): { last8: string; last9: string } {
  const normalized = normalizePhoneE164(phone);
  return {
    last8: normalized.slice(-8),
    last9: normalized.slice(-9),
  };
}
